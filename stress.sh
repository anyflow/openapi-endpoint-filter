...
URL="https://api.anyflow.net/dockebi/v1/stuff"
...
while [ $qps -le $MAX_QPS ]; do
    ...
    # Fortio 실행 (JSON 결과 저장)
    fortio load -qps $qps -c $THREADS -t $DURATION -json result.json $URL

    # JSON에서 평균 응답 시간 추출
    avg_latency=$(jq '.DurationHistogram.Avg' result.json)

    # HTTP 500, 503 에러 발생 횟수 추출
    ...
    echo "Avg Latency: ${avg_latency}s, Error Rate: ${error_rate}%"
